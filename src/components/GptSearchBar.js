...
const GptSearchBar = () => {
  ...
  const fetchMoviesByGenre = async (genreIds) => {
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&with_genres=${genreIds}&language=en-US&&sort_by=popularity.desc`
      );
  ...
  const handleGptSearchClick = () => {
    const inputGenres = searchText.current.value
      .split(",")
      .map((g) => g.trim().toLowerCase());
    const genreIds = inputGenres
      .map((inputGenre) => {
        const genre = genresData.genres.find(
          (g) => g.name.toLocaleLowerCase() === inputGenre
        );
